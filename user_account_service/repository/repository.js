...
class Repository {
  ...
  getUsers() {
    return new Promise((resolve, reject) => {
      ...
      this.connection.query('SELECT id, username, phone_number FROM user_profile', (err, results) => {
        if(err) {
          this.connection = mysql.createConnection(this.connectionSettings);
  ...
  getUserByUsername(username) {
    ...
    return new Promise((resolve, reject) => {
      ...
      //  Fetch the customer.
      this.connection.query('SELECT id, username, phone_number FROM user_profile WHERE username = ?', [username], (err, results) => {
